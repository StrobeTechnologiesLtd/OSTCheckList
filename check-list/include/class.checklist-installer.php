...
class CheckListInstaller extends SetupWizard {
	...
	function install() {
		$schemaFile = CHECKLIST_PLUGIN_ROOT . 'install/sql/install_checklist.sql'; // DB dump.
		return $this->runJob ( $schemaFile );
	}
	
	function remove() {
		$schemaFile = CHECKLIST_PLUGIN_ROOT . 'install/sql/remove_checklist.sql'; // DB dump.
		return $this->runJob ( $schemaFile );
	}
